...
var _WIDTH;
var _HEIGHT;
var _DISABLED_SERVER_COMM = false;
...
disableServerComm = function () {
    _DISABLED_SERVER_COMM = true;
};

enableServerComm = function () {
    _DISABLED_SERVER_COMM = false;
};
...
addFunctionOptions = function ($parent, cif, name, value, parOptions, width) {
    ...
    var $f = addRadio($leftSpan, cif, 1, cif + "-" + name + "-radio", ["F", "Funcionalidade"], value <= 4).change(function () {
    ...
    var $i = addRadio($rightSpan, cif, 2, cif + "-" + name + "-radio", ["I", "Incapacidade"], value >= 5).change(function () {
    ...
    $div.css({
        width: '412px', 'background-color': 'white', overflow: 'hidden', height: '38px',
        'text-align': 'left'
    });
    $div.buttonset();
...
addStructureItem = function (node, cif, text, values) {
    ...
    // Descrição
    tailText.text(text).addClass("ui-button-text").css({
        'white-space': 'nowrap',
        ...
        $.get(address, function (group) {
            ...
            if (results.length == items.length) {
                results.sort(function (item1, item2) {
                    return item1.cif.localeCompare(item2.cif);
                });
